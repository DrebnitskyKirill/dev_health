...
router.post("/achievement", auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const notificationService = new notification_1.NotificationService();
        const success = await notificationService.sendAchievementEmail(user, achievementName, achievementDescription);
        if (success) {
...
router.post("/daily-reminder", auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user?.userId;
        if (!userId) {
            return res.status(401).json({ message: "User not authenticated" });
...
router.post("/weekly-report", auth_1.authenticateToken, async (req, res) => {
    try {
...
router.get("/templates", auth_1.authenticateToken, (req, res) => {
    try {
        const templates = notification_1.NotificationService.getTemplates();
